...
import { useLocation } from 'react-router-dom'; // lol wtf o hook din packetul react-router-dom, fol pt a ne spune unde e locatia in browser
...
const Review = (props) => {
    ...
    useEffect(()=>{
        window.scrollTo(0,0); // scroll la primul rand cand se schimba url pt fiecare pagina noua
    },[pathname])
    ...
    useEffect(()=>{
        dispatch(getReviewById(props.match.params.id)).then((response)=>{
            if(!response.payload){
                props.history.push('/');
            } // daca id'ul nu exita il trimite in alta pagina
        })
    },[props,dispatch]);
